...
ensure_featmake() {
  if ! type featmake >/dev/null 2>&1; then
    ...
    (
      cd $temp_dir
      sha256sum --check --strict $temp_dir/checksums.txt
    )
...
featmake "ghcr.io/devcontainers-contrib/features/asdf-package:1" -PLUGIN "asdf-plugin-manager" -PLUGINREPO "https://github.com/asdf-community/asdf-plugin-manager.git" -VERSION "$PLUGIN_MANAGER_VERSION"

cat <<EOF >>/home/vscode/.plugin-versions
$PLUGIN              $PLUGINREPO                                                $REF
asdf-plugin-manager  https://github.com/asdf-community/asdf-plugin-manager.git  $PLUGIN_MANAGER_REF
EOF
